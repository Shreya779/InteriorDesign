...
$sql = "SELECT * FROM form_bedroom_room ORDER BY id DESC LIMIT 1"; // Assuming id is the primary key and auto-incremented
...
if ($result->num_rows > 0) {
    ...
    $width = $row["width"];
    $length = $row["length"];
    $sofa_type = $row["bed_type"];
    $furniture = $row["furniture"];
    $lighting = $row["flooring"];
    $paint = $row["paint"];
    ...
    if ($result_user->num_rows > 0) {
        ...
        echo "<p><strong>Bed type:</strong> $sofa_type</p>";
        echo "<p><strong>Furniture type:</strong> $furniture</p>";
        echo "<p><strong>Flooring type:</strong> $lighting</p>";
